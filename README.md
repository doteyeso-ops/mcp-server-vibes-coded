...
- **IDE sidecar (v1.7.1):** tool results include `print_to_user` on 402 — show it to the human. Local stdio auto-claims a smoke trial once. Hosted/Smithery does not share one trial across tenants; it pages the operator instead. Boot fix: do not declare `global` after reading module state.
